...
export async function adjustScore(req, res) {
  ...
  try {
    ...
    if (!user) {
      await session.abortTransaction(); return res.status(404).json({ ok: false, msg: '用户不存在' })
    }
    if (user.score + amount < 0) {
      await session.abortTransaction(); return res.status(400).json({ ok: false, msg: '积分不足，无法扣减至负数' })
    }
    ...
    // 【新增】：事务成功后，通过 WS 个人频道精准推送最新积分
    const io = getIO()
    if (io) {
      io.to(`user_${req.params.id}`).emit('score_sync', { newScore: user.score + amount })
    }
...
export async function audit(req, res) {
  try {
    ...
    res.status(500).json({ ok: false, msg: '审计查询失败' })
  }
}
