...
function switchBGM(name) {
  // 如果点击的是当前正在播放的，就传 null 关闭；否则传新的名字
  const targetName = name === currentBGM.value ? null : name
  // 接收 audioManager 返回的真实状态，保持前端按钮高亮同步
  currentBGM.value = audioManager.switchBGM(targetName)
}

// ==========================================
// 8. 生命周期钩子
// ==========================================
let hasJoinedRoom = false // 【新增】防止重复加入房间的锁

onMounted(() => {
  const s = getSocket()
  
  // 如果根本没有 socket 实例（比如没登录），直接踢走
  if (!s) {
  ...
  const attemptJoin = () => {
    ...
    s.emit('join_room', { roomId: route.params.roomId }, (res) => {
      if (!res.ok) {
        // 【关键修复】：如果后端提示"你还在游戏中"，不要傻乎乎地报错踢走！
        // 自动帮你走断线重连的恢复流程
        if (res.code === 'IN_GAME_REDIRECT' && userStore.id) {
          s.emit('reconnect_room', { userId: userStore.id }, (reRes) => {
            if (reRes?.ok === false) {
  ...
  if (s.connected) {
    ...
    // 【关键修复】：情况 B：刷新页面导致断线中！
    // 不要踢走，主动推一把重连，并监听成功事件
    showToast('正在恢复网络连接...', 'info')
    s.connect() 
    
    const onReconnectSuccess = () => {
      s.off('connect', onReconnectSuccess)
      // 重连成功后，再去执行进房间的动作
      attemptJoin()
    }
    s.on('connect', onReconnectSuccess)

    // 兜底：如果 5 秒后还是没连上，才认输踢回大厅
    setTimeout(() => {
      if (!hasJoinedRoom) {
  ...
  // 断线重连底层监听，自动恢复房间状态 (保留原逻辑，但加了限制)
  s.on('connect', () => {
    // 只有在“已经成功进入过房间”的情况下，中途断线又连上，才自动发重连请求
    if (hasJoinedRoom && userStore.id && roomData.value.roomId) {
