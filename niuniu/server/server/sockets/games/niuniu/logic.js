    ...
    for (let j = i + 1; j < 5; j++)
      for (let k = j + 1; k < 5; k++) {
        if ((getCardPoint(hand[i]) + getCardPoint(hand[j]) + getCardPoint(hand[k])) % 10 === 0) {
          const r = []
          for (let m = 0; m < 5; m++) if (m !== i && m !== j && m !== k) r.push(hand[m])
          const s = (getCardPoint(r[0]) + getCardPoint(r[1])) % 10
          if (s === 0) return { result: 'bull_bull', rank: 10, type: 'bull_bull' }
          return { result: `bull_${s}`, rank: s, type: 'bull' }
        }
      }
