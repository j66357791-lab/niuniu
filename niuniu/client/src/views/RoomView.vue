...
const bubbles = ref([]) // { id, userId, text }
...
const isMyTurnShowdown = computed(() => {
  ...
  if (phase === 'playing') {
    return !isSpectator.value && userStore.id !== roomData.value.bankerId && !bullResults.value[userStore.id]
  }
...
function startPingLoop() {
  ...
  pingTimer = setInterval(() => {
    if (s.connected) s.emit('ping', { timestamp: Date.now() })
  }, 3000)
}
...
function triggerBanner(results) {
  ...
  if (bannerInfo.value) {
    clearTimeout(bannerTimer); bannerTimer = setTimeout(() => { bannerInfo.value = null }, 3500)
  }
}
...
function handleBankerSelected(data) {
  stopRobHighlight(); roomData.value.bankerId = data.bankerId; robbers.value = data.robbers || []
}
...
function handleGameResult(data) {
  ...
  if (my) {
    ...
    my.scoreChange >= 0 ? audioManager.play('win') : audioManager.play('lose')
  }
...
function stopRobHighlight() {
  if (robHighlightTimer) { clearInterval(robHighlightTimer); robHighlightTimer = null }
}
...
function startFlyingAnimation(results) {
  ...
  results.forEach((r, i) => {
    ...
    if (r.scoreChange > 0) {
      flies.push({ id: `f${i}`, text: `+${r.scoreChange.toFixed(2)}`, color: '#22a85e', x: bankerPos.x, y: bankerPos.y, endX: pp.x, endY: pp.y, faded: false })
    } else {
      flies.push({ id: `f${i}`, text: `${r.scoreChange.toFixed(2)}`, color: '#c04a4a', x: pp.x, y: pp.y, endX: bankerPos.x, endY: bankerPos.y, faded: false })
    }
  })
...
function sendBubble(idx) {
  const s = getSocket(); if (!s?.connected) return; s.emit('chat_bubble', { bubbleId: idx })
}
...
function updateAudioVolume(val) {
  audioVolume.value = val; audioManager.setVolume(val)
}

function toggleAudioMute() {
  audioMuted.value = !audioMuted.value; audioManager.toggleMute()
}

function switchBGM(name) {
  audioManager.switchBGM(name); currentBGM.value = name === currentBGM.value ? null : name
}
...
onMounted(() => {
  ...
  s.on('pong', (data) => { pingMs.value = Date.now() - data.timestamp }); startPingLoop()
...
onUnmounted(() => {
  const s = getSocket()
  if (s) {
    s.off('pong'); s.off('room_update', handleRoomUpdate); s.off('phase_changed', handlePhaseChanged);
    s.off('countdown', handleCountdown); s.off('banker_selected', handleBankerSelected);
    s.off('room_notice', handleRoomNotice); s.off('deal_cards', handleDealCards);
    s.off('deal_cards_private', handleDealCardsPrivate); s.off('player_showdown', handlePlayerShowdown)
    s.off('bet_choice', handleBetChoice); s.off('banker_double_choice', handleBankerDoubleChoice);
    s.off('game_result', handleGameResult); s.off('chat_bubble', handleChatBubble);
    s.off('box_appear', handleBoxAppear); s.off('box_grabbed', handleBoxGrabbed);
...
<template>
  <div class="min-h-screen flex flex-col select-none">
          ...
          <div v-for="(seat, idx) in seats" :key="idx" :class="[seatPosition[idx] || 'flex items-center justify-center', 'flex flex-col items-center justify-center']">
            ...
            <template v-else>
              <div class="w-[100px] sm:w-[120px] h-[120px] sm:h-[140px] rounded-xl border flex flex-col items-center py-0.5 px-0.5 sm:py-1 sm:px-1 transition-all duration-100 relative" :class="[seat.offline ? 'bg-table-bg/60 border-table-border opacity-60' : seat.isMe ? 'bg-table-panel border-gold/40' : 'bg-table-panel border-table-border', isRobAnimating && seat.isRobber && robbers[robHighlightIdx] === seat.userId ? 'rob-highlight' : '']">
                ...
                <!-- 【终极修复：前3张 + 后2张 永远5张，且数组索引绝对精准】 -->
                <div class="flex items-center justify-center flex-1 w-full">
                  <template v-if="roomData.phase === 'waiting'"></template>
                  <template v-else>
                    <!-- 前3张位置 (ci从1到3，减1变成0到2对应数组前三项) -->
                    <div v-for="ci in 3" :key="'f-'+seat.userId+'-'+ci" 
                         class="w-[16px] h-[22px] sm:w-[20px] sm:h-[28px] relative shrink-0" 
                         :style="{ marginLeft: ci > 0 ? '-5px' : '0', zIndex: ci }">
                    ...
                    <!-- 后2张位置 (ci从1到2，减1变成0到1对应数组后两项) -->
                    <div v-for="ci in 2" :key="'pr-'+seat.userId+'-'+ci" 
                         class="w-[16px] h-[22px] sm:w-[20px] sm:h-[28px] flip-card relative shrink-0" 
                         :class="{ flipped: !!revealedCards[seat.userId] }"
                         :style="{ marginLeft: '-5px', zIndex: ci + 3, '--flip-delay': (ci - 1) * 0.15 + 's' }">
    ...
    <div v-for="fly in flyingScores" :key="fly.id" class="fixed z-[200] text-lg sm:text-xl sm:text-2xl font-bold pointer-events-none fly-score" :class="{ 'fade-out': fly.faded }" :style="{ left: fly.x + 'px', top: fly.y + 'px', color: fly.color, transform: 'translate(-50%, -50%)' }">{{ fly.text }}</div>
...
.flip-card.flipped .flip-card-inner {
  transform: rotateY(180deg); transition-delay: var(--flip-delay, 0s);
}
...
.no-bull-text {
  color: #7a8a7e !important; font-weight: 800;
  text-shadow: 1px 1px 0 rgba(0,0,0,0.9), -1px -1px 0 rgba(0,0,0,0.9), 1px -1px 0 rgba(0,0,0,0.9), -1px 1px 0 rgba(0,0,0,0,9), 0 0 6px rgba(0,0,0,0.5);
}
</style>
