...
async function forceSettleBox(io, room) { 
  ...
  // 【修复精度】从整数转回元去加积分
  const refund = box.remainingCents / 100
...
function resetRoom(io, room) { 
  // 【修复问题9】牛牛专属定时器由牛牛 handler 自己清理，解耦 baseState
...
export function niuniuHandler(socket, io) { 
  ...
  socket.on('create_box', async ({ totalAmount, maxGrabbers }, callback) => { 
    ...
    try { 
      ...
      room.currentBox = { 
        ...
        maxGrabbers, 
        grabbedList: [], 
        remainingCents: totalCents, // 核心字段：用整数记录剩余
        remaining: totalAmount,     // 保留供前端直接展示
        isFinished: false, 
        timer: null 
      } 
  ...
  socket.on('grab_box', (_, callback) => { 
    ...
    // 【修复精度】直接用整数进行随机和扣减，不乘不除，绝对精准
    const remCents = box.remainingCents
    ...
    // 转回元给前端展示和记录
    const actualGrab = grabCents / 100 
    
    // 整数精准扣减
    box.remainingCents -= grabCents 
    box.remaining = box.remainingCents / 100 
